...
import { initializeEntity } from "../shared/entity";
...
function main(): void {
  ...
  const playerEntityDisplay = initializeEntity(undefined, 50, 50, appContainer, cache);
}
...
function main2(): void {
  ...
  bar.x = 50;
  bar.y = 45;
  appContainer.addChild(bar);

  // resource bars
  barRed.x = 50;
  barRed.y = 200;
  barRed.width = 0;
  appContainer.addChild(barRed);
  barGreen.x = 50;
  barGreen.y = 250;
  barGreen.width = 0;
  appContainer.addChild(barGreen);
  barYellow.x = 50;
  barYellow.y = 300;
  barYellow.width = 0;
  appContainer.addChild(barYellow);
  ...
  appContainer.addChild(barRedE);
  barGreenE.x = 300;
  barGreenE.y = 250;
  barGreenE.width = 0;
  barGreenE.visible = false;
  appContainer.addChild(barGreenE);
  barYellowE.x = 300;
  barYellowE.y = 300;
  barYellowE.width = 0;
  barYellowE.visible = false;
  appContainer.addChild(barYellowE);
...
function onDragEnd(event: PIXI.interaction.InteractionEvent) {
  if (draggingObj !== undefined) {
    ...
    if (prevHitIndex !== undefined) {
      ...
      nodes[prevHitIndex].texture = cache[nodeSprite((draggingObj as any).template)];
    }
